...
token: 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
...
| 时间              | 事情                                                         |
| ----------------- | ------------------------------------------------------------ |
| 研二上（-1.10）   | 1.写专利，修够学术分【OK】；2.跑毕设实验；                   |
| 寒假（1.10-2.20） |                                                              |
...
1.ICML2022会议[CallForPapers](https://icml.cc/Conferences/2022/CallForPapers)【Failed，还需要继续做实验】

摘要结稿日期Jan 20, 2022 11:59 PM AOE ；

结稿日期Jan 27, 2022 AOE；

页数：正文最长可达8页，参考文献和附录不限页数；

最好有匿名github代码链接；

1月3号-1月9号写好初稿，后面给老师改；

1月10号-1月16号，增加num_iters跑cat和EncDecInv直到收敛，实验结果画图；(尝试画120000的图)
...
4. 主要任务
   ...
   （1）重新设置环境batch size和num_iter_steps，恢复原来的设置，跑实验，希望能收敛；

   （2）将EncDecInv框架放到BC/GAIL上跑实验；

   如果专家轨迹给的少，是不是训练不好EncDecInv框架，对于BC实验，可以多给一点专家轨迹进行训练；

   （3）GAIL是可以画出训练图的，不过很多都会用success rate，但是感觉和avgret差不多。重复10次实验得到方差图。
...
- 今日工作：
  ...
  1. 当前主要工作
     ...
     ![image-20220107212532331](/home/hehua/.config/Typora/typora-user-images/image-20220107212532331.png)

  2. 理解core-set方法：每轮迭代查找距离当前有标记样本集最远的一批样本；

  3. BC_EncDecInv

     （1） 预训练EncDecInv（输入专家的st, st+1, at）；

     （2）BC输入改为提取到的policy embedding（参考CAT的discriminator）；

     （3）把预训练EncDecInv，嵌入到run_BC中；
...
- 今日工作：
  ...
  2. 跑door实验
     ...
     本地跑cat，估计5h，下午16:24跑完；【跑了8h】

     本地跑cat（120000），估计32h，大后天凌晨4:55跑完；

  3. MY_AL_Learning库无法push的问题

     （1）重新配置ssh密钥

     ```bash
     ssh config --global user.name "*"
     ssh config --global user.email "*@qq.com"
     ssh config --list
...
- 明日安排：
  ...
  1. 梳理HalfCheetah（num-iters）30000, 60000, 120000曲线图；
  2. 分析后续实验重点；
  3. 引言；
...
- 今日工作：
  ...
  1. 跑door实验
     ...
     cat_BC：估计2h，11:49跑完；【OK】

     EncDecInv:$\lambda= \beta=[0.1, 0.2, 0.3]$​九组BC实验，估计跑6*3=18h，凌晨28:14=4:14跑完；【Failed】

     EncDecInv:$\lambda=[0.1, 0.2, 0.3], \beta=[0.5, 0.7, 0.9]$九组实验，估计跑15h,凌晨24：50=00:50跑完；【OK,4:40=>19h】

     192.168.3.6上跑$\lambda=[0.5, 0.7, 0.9], \beta=[0.2]$​三组实验，估计跑7h，晚上22：23跑完；

     192.168.3.6上跑$\lambda=[0.5, 0.7, 0.9], \beta=[0.1]$​三组BC实验，估计跑2h，中午12:13跑完；(PID=159646);
...
- 今日工作：
  ...
  1. 下载hammer/pen/door的专家轨迹

     [参考论文 Learning Complex Dexterous Manipulation with Deep Reinforcement Learning and Demonstrations](https://sites.google.com/view/deeprl-dexterous-manipulation)

     论文里说可以用NPG（mjrl中的算法）来学习这几个环境，效果稍微好点，但是既然有专家轨迹，我就不重新训练了。

     [轨迹下载github](https://github.com/aravindr93/hand_dapg)

  2. 生成专家轨迹，跑cat/EncDecInv;

  3. 跑door实验

     cat：估计跑5h，下午16:41跑完；

     EncDecInv:$\lambda= \beta=[0.1, 0.2, 0.3]$​​​​九组实验，估计跑15h,凌晨26：47=2:47跑完；

     192.168.3.6上跑$\lambda=[0.5, 0.7, 0.9], \beta=[0.1]$​​三组实验，估计跑7h，晚上22：23跑完；
...
2. 确定后续实验（12/29）；
3. 拿居住证（12/31）；
4. 挑战杯培训沙龙（12/30 19:00）；

#### 2021/12/31

- 今日工作：

  1. 在192.168.3.6上，安装mj_envs环境；

  2. 在本地安装mj_envs环境；

     [git clone mj_envs](https://github.com/vikashplus/mj_envs)

     [下载mjrl](https://github.com/aravindr93/mjrl/tree/master/setup#installation)
     ...
     然后，参考[GLEW error](https://blog.csdn.net/gsww404/article/details/80636676)
     ...
     ```bash
     conda install pytorch==1.4.0 # https://pytorch.org/get-started/previous-versions/
...
- 今日工作：
  ...
  1. 跑InvertedPendulum实验

     cat & BC：估计跑2h，12:21跑完；【OK】

     EncDecInv1.0-1.0 & BC：估计跑2h，12:27跑完；【OK】

  2. 写专利（文字写完了，到word里面改一下图片应该就行）；
...
- 今日工作：
  ...
  1. 192.168.3.6上跑Hopper($\lambda=1, \beta=1$​)BC实验，估计跑2h，12：12跑完（PID：56230）；

  2. 尝试跑InvertedPendulum-v2（RL）实验；

  3. 写专利；

  4. 跑InvertedPendulum实验（BC，cat，EncDecInv1.0-1.0）；

     BC：跑完了 ；

     cat：估计跑5h，21:00跑完；

     EncDecInv1.0-1.0：估计跑5h，21:00跑完；

     cat & BC：估计跑2h，24:17跑完；

- 明日安排：

  1. 可以尝试画augret随迭代次数变化的图，因为每次BC验证都是100条轨迹验证，所以感觉应该可以得到一个图？
...
- 今日工作：
  1. 统计Hopper个i-lambda权重和i-beta权重结果；
  2. 192.168.3.6上跑Hopper($\lambda=1, \beta=1$​)，估计跑5h，凌晨26:20=2:20跑完；
- 明日安排：
  1. 可以尝试跑InvertedPendulum-v2（RL）实验；
  2. 写专利；

### 下周工作安排

1. 看论文，找思路；
2. 修改代码，能够从停止的迭代次数，加载模型继续开始训练；
3. 通知团队成员每个人写一个主动学习相关的专利；
4. 理解HalfCheetah环境的状态维度信息，appearance embedding一定要相似吗？
...
- 今日工作：
  ...
  1. 跑Hopper实验
     ...
     本地跑$\lambda=[0.5, 0.7, 0.9], \beta=[0.7, 0.9]$​九组BC实验，估计跑6*2=12h，晚上23:51跑完；
...
- 今日工作：
  ...
  1. 跑Hopper实验
     ...
     本地跑$\lambda=[0.1, 0.2, 0.3], \beta=[0.5, 0.7, 0.9]$​九组BC实验，估计跑6*3=18h，凌晨27:43=3:43跑完；

     本地跑$\lambda=[0.5, 0.7, 0.9], \beta=[0.7, 0.9]$​九组实验，估计跑15h，晚上24:43跑完；

     192.168.3.6上跑$\lambda=[0.5, 0.7, 0.9], \beta=[0.1]$​三组BC实验，估计跑6h，下午15:49跑完（PID：16014）；

     192.168.3.6上跑$\lambda=[0.5, 0.7, 0.9], \beta=[0.2]$​三组BC实验，估计跑6h，下午15:51跑完（PID：16212）；

     192.168.3.6上跑$\lambda=[0.5, 0.7, 0.9], \beta=[0.3]$​三组BC实验，估计跑6h，下午15:51跑完（PID：16386）；

     192.168.3.6上跑$\lambda=[0.5, 0.7, 0.9], \beta=[0.5]$​三组BC实验，估计跑6h，下午15:51跑完（PID：16644）；
...
- 今日工作：
  ...
  1. 跑Hopper实验
     ...
     本地跑$\lambda=[0.1, 0.2, 0.3], \beta=[0.5, 0.7, 0.9]$​​​九组实验，估计跑15h，晚上24:12跑完；

     本地跑$\lambda=[0.1, 0.2, 0.3], \beta=[0.1, 0.2, 0.3]$九组BC实验，估计跑6*3=18h，凌晨28:14=4：14跑完；

     192.168.3.6上跑$\lambda=[0.5, 0.7, 0.9], \beta=[0.3]$​​​​​三组实验，估计跑7h，下午16:23跑完；=>17：00跑完；

     192.168.3.6上跑$\lambda=[0.5, 0.7, 0.9], \beta=[0.1]$​三组BC实验，估计跑6h，下午16:23跑完（PID：11265）；=>Failed

     192.168.3.6上跑$\lambda=[0.5, 0.7, 0.9], \beta=[0.5]$​​​三组实验，估计跑7h，下午24:00跑完；
...
- 今日工作：
  ...
  2. 和师兄讨论实验结果；
     ...
     建议按照老师的思路，先跑重复实验，和其他环境的实验，赶一下IJCAI；

     问题：

     （1）重复实验，是重跑cat/EncDecInv，再BC吗？还是只重跑BC

     应该是重跑cat/EncDecInv，再BC。因为之前应该是设的随机种子同，同一个cat模型BC跑出的结果一样；
  ...
  3. 跑Hopper实验
     ...
     本地跑$\lambda=[0.1, 0.2, 0.3], \beta=[0.1, 0.2, 0.3]$​九组实验，估计凌晨25:16=1:16跑完；

     1:47跑完的，用了11h；

     192.168.3.6上跑$\lambda=[0.5, 0.7, 0.9], \beta=[0.1]$​​三组实验，已跑完；

     192.168.3.6上跑$\lambda=[0.5, 0.7, 0.9], \beta=[0.2]$​三组实验，估计凌晨29:47=5:47跑完；
  ...
  4. 主要工作：
     ...
     （2）Hopper和cat相关的EncDecInv实验；（12/22-）
...
- 今日工作：
  1. 跑$\lambda=[0.5, 0.7, 0.9], \beta=[0.1, 0.2, 0.3, 0.5, 0.7, 0.9]$​BC实验；
  2. 小挑初赛；
- 明日安排：
  1. 总结实验结果；
  2. 修改专利；
...
- 今日工作：
  ...
  1. 查看i_lambda_beta BC结果

     |         | generator  |    aug_avgret     | aug_avglen |      avgret       |    success ratio    | avglen |
     | :-----: | :--------: | :---------------: | :--------: | :---------------: | :-----------------: | :----: |
     | expert  |     ——     | 2432.090276928705 |   1000.0   | 968.1854731821866 | 21.865217104310563% | 1000.0 |
     |   cat   | iter_2000  |    595.7065548    |    1000    |    1347.141463    |       30.82%        |  1000  |
     |   MSE   | iter_3000  |    1102.929526    |    1000    |  **1432.70937**   |       32.10%        |  1000  |
     | 0.2-0.3 | iter_3000  |    1147.407651    |    1000    |    1579.577582    |       35.81%        |  1000  |
     | 0.3-0.3 | iter_2000  |    1283.617095    |    1000    |    2005.447011    |       44.93%        |  1000  |
     | 0.2-0.9 | iter_20000 |    944.9944095    |    1000    |    1576.612516    |       35.47%        |  1000  |
     | 0.9-0.1 | iter_12000 |    981.0612634    |    1000    |    1496.018771    |       33.52%        |  1000  |

  2. 跑$\lambda=[0.5, 0.7, 0.9], \beta=[0.5, 0.7, 0.9]$​九组实验;

     估计21号凌晨25:59=1:59跑完；

  3. 跑$\lambda=[0.1, 0.2, 0.3], \beta=[0.5, 0.7, 0.9]$BC实验；
  ...
  5. 和老师讨论实验结果：
     ...
     看看能不能直接用于模仿学习，不借助cat，说明我们方法的效果；

     后续多跑几个环境看看，做些重复实验看看稳定性；
...
1. 1月16号去拿居住证；
2. 实现老师提议的想法（12/13-12/14）；
3. 跑cat论文环境对比实验（12/15）；
4. 12月24号之前给唐英鹏师兄弄好专利修改（12/16-12/17）；
5. 12月21号，小挑展示，20号发PPT到邮箱（12/17-12/18）；

#### 2021/12/19

- 今日工作：

  1. 跑$\lambda=[0.5, 0.7, 0.9], \beta=[0.1, 0.2, 0.3]$​​九组实验;
  ...
  2. 从训练曲线来看
     ...
     | $\lambda$ | $\beta$ |        结果         |
     | :-------: | :-----: | :-----------------: |
     |    0.3    |   0.1   | True_rewards比MSE好 |
     |    0.2    |   0.5   |         ——          |
     |    0.1    |   0.7   |         ——          |
     |    0.2    |   0.9   |         ——          |

     ![image-20211219162843906](/home/hehua/.config/Typora/typora-user-images/image-20211219162843906.png)
...
- 今日工作：
  ...
  1. 跑120000实验（昨天跑的实验，早上8:20竟然停掉了，iters=65000）;

     续跑问题亟待解决～

     EncDecInv_num-iters-120000_HalfCheetah:3473;

     MSE_num-iters-120000_HalfCheetah: 3305;

     cat_num-iters-120000_HalfCheetah: 3108;

     预计跑15h，大概18号凌晨1:14跑完；

  2. 整理num-iters-60000实验结果，和30000相似；

  3. 熟悉小挑PPT；

  4. 和师兄讨论实验结果

     （1）apparence loss，reconstraction loss再缩小10倍，因为从损失曲线来看，EncDecInv的inverse loss比policy-embedding-MSE的inverse loss大，有可能是被apparence loss和reconstraction loss带跑偏了。
     $$
     loss=mse(\hat{a}_t, a_t)+\lambda cosin(E_A(s_t),E_A(s_{t+1}))+\beta L_1(Decoder(E_A(s_t), E_P(s_t)))
     $$
     网格搜索或者随即搜索超参$\lambda=[0.1, 0.2, 0.3,......], \beta=[0.1, 0.2, 0.3,......]$​​​，预期影响比较大；

     （2）其余环境Ant/Hopper也要调一下其他参数，影响估计也挺大；

  5. 先跑$\lambda=[0.1, 0.2, 0.3], \beta=[0.1, 0.2, 0.3]$​九组实验看看；

     后面可以再跑一下$\lambda=[0.1, 0.2, 0.3, 0.5, 0.7, 0.9], \beta=[0.1, 0.2, 0.3, 0.5, 0.7, 0.9]$​
...
- 明日安排：
  ...
  2. 跑BC和$\lambda=[0.5, 0.7, 0.9], \beta=[0.1, 0.2, 0.3]$​实验；
...
- 今日工作：
  ...
  1. BC
     ...
     HalfCheetah_cat_num-iters-60000;

     HalfCheetah_policy-embedding-MSE_num-iters-60000;

     HalfCheetah_policy-embedding-EncDecIn_num-iters-60000;

     基本8min/个模型，估计跑8h，估计21：40跑完；

     还需要3h，估计24:08跑完；

  2. 中午走之前跑Ant_cat_hidden-size-256_BC;

  3. 整理HalfCheetah_policy-embedding-EncDecInv_num-iters-30000实验结果；

  4. 配环境：驱动460->cuda11.0->cudnn8.0->anaconda3->tensorflow-gpu1.14->mpi4py->baselines->mujoco->yaml
     ...
     [安装mpi4py模块遇到的一些问题（Ubuntu）](https://blog.csdn.net/weixin_43255962/article/details/103681400)
  ...
  5. 跑120000实验；
     ...
     EncDecInv_num-iters-120000_HalfCheetah: 60262;

     MSE_num-iters-120000_HalfCheetah: 60878;

     cat_num-iters-120000_HalfCheetah: 61399;

     预计跑15h，大概17号20:54+17:00=37:54->下午13:54跑完；
...
- 今日工作：
  ...
  1. 实现老师提议的想法Enc_Dec_Inv；

     本地跑，估计16:42跑完；

     BC,估计21：17跑完；

  2. cat_Ant_hidden-size-256：估计20:33跑完；

     应该会比原来hidden-size-64（num-iters-30000）好；

  3. 感觉EncDecInv还能提升，改为num_iters=60000试试；
...
- 今日工作：
  ...
  1. tanh&BC，估计跑2h，11:39跑完；

  2. HalfCheetah_cat-policy-embedding_sigmoid&BC: 估计跑2h，12:44跑完；

  3. HalfCheetah_cat-policy-embedding_phi-gaussian-True_sigmoid_ac-gaussian_sigmoid: 估计跑5h，14:45跑完；

  4. 明白了为什么服务器上比本地跑的慢了，因为本地ubuntu我是装在固态硬盘上的，而服务器上估计是机械硬盘；

     服务器上跑的时间基本是本地耗时的一倍，本地跑5h，服务器上要9.5h；

  5. Ant学不好的原因似乎有点思路了，因为我们设置的discriminator/policy/inverse model的隐藏层都是64，而Ant的状态维度是111维（HalfCheetah是17维，Hopper是11维），中间损失了太多信息，对Ant来说，似乎隐藏层大小应该设大一些；

  6. 学习党课，十九届六中全会精神（曲青山院长）；
...
- 今日工作：
  ...
  1. 昨天214上跑的代码：15:11-9：19，有18h了；

     cat-Ant：iters=32731，平均30iter/min（本地跑100iter/min，比本地慢了2/3啊～）

     cat-Ant-g-step-6：iters=16300，平均15iter/min（比本地慢了5/6啊～）

     这样算的话，cat/policy embedding估计要跑34h，大概14号凌晨1点跑完；

     cat/policy embedding_g-step-6估计要跑67h，大概15号上午10点跑完；
  ...
  2. 跑policy embedding:
     ...
     发现互信息与MSE不在一个量级，后面可以设置权重进行缩放一下（用relu将值缩放到[0,无穷)）；

     感觉用relu也不太合理(而且从训练曲线发现，互信息大部分时间都小于0)，因为H(z)固定，我最小化I(S,Z)，就是在最大化H(Z|S)，本质上也就是在最大话P(Z|S)的方差，因此我需要惩罚方差小的情况，relu只惩罚了一部分（互信息小于0的那大部分没有做惩罚），所以可以尝试sigmoid

     HalfCheetah_cat-policy-embedding_num-iters-30000_phi-gaussian-True_sigmoid:3256045（209）
...
- 明日安排：
  ...
  2. 尝试把A也假设成一个分布，来优化损失函数

     $loss = -I(A;Z)+I(S,Z)+MSE(A, A^*)$​

  3. 跑sigmoid和ac-gaussian；


### 周工作安排（12/6-12/10）

1. 缩短训练周期（12/6）【OK】；
2. 调整policy embedding（12/7-12/8）【OK】；
3. 12月10号去拿暂住证。
...
- 今日工作：
  ...
  1. 重跑cat/policy embedding实验（214）；

     HalfCheetah_cat_num_iters-60000: 1611936

     HalfCheetah_cat-policy-embedding_num_iters-60000:1609920

     Hopper_cat_num_iters-60000: 1610356;

     Hopper_cat-policy-embedding_num_iters-60000: 1607209

     Ant_cat_num_iters-60000: 1610909

     Ant_cat_num_iters-60000_g-step-6: 1611513

     Ant_cat-policy-embedding_num_iters-60000: 1607770

     Ant_cat-policy-embedding_num_iters-60000_g-step-6: 1609392

     估计要跑10h，大概13号凌晨1:11跑完；Ant_g-step-6应该会要20h，大概13号中午11:11跑完；
